	...
	public class NavigationTest : TestBase
	{
		...
		*/
		public void TryFindPage()
		{
			...
			TFPCheck(p.P1, p.Inner1, p.P1, null);
			TFPCheck(p.E1, p.Inner1, p.P1, null);
			TFPCheck(p.P2, p.Outer, p.P2, null);
			TFPCheck(p.E2, p.Outer, p.P2, null);
			TFPCheck(p.E3, p.Outer, p.P2, p.E3);
			TFPCheck(p.E4, p.Outer, p.P2, p.E3);
			
			TFPCheck(p.DP1, p.Deep1, p.DP1, null);
			TFPCheck(p.P4, p.Inner2, p.P4, null);
		}
		
		void TFPCheck(Visual page, INavigation navObject, Visual pageObject, Visual pageBindObject,
			[CallerFilePath] string filePath = "", [CallerLineNumber] int lineNumber = 0,
			[CallerMemberName] string memberName = "")
		{
			_navReady = false;
			var npp  = new NavigationPageProxy(NavReady, NavUnready);
			npp.Rooted(page);
		...
		*/
		...
		{
			var p = new UX.PageControlActivated();
			var root = TestRootPanel.CreateWithChild(p);
		...
		{
			var p  = new UX.RouterNavigatorActivated();
			var root = TestRootPanel.CreateWithChild(p);
